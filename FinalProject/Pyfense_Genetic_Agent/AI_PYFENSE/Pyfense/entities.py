...
class PyFenseEntities(cocos.layer.Layer, pyglet.event.EventDispatcher):
    ...
    def __init__(self, path, startTile, endTile, is_display_genome_mode_on):
        super().__init__()
        self.is_display_genome_mode_on = is_display_genome_mode_on
        self.enemies = []
    ...
    def on_has_enemy_died(self, target):
        ...
        if target in self.enemies and target.healthPoints <= 0:
            ...
            #clock.schedule_once(lambda dt, x: self.remove(x), 0.5, deathAnimation)
            self.diedEnemies += 1
    ...
    def _add_enemy(self, dt):
        ...
        # if display_genome_mode, do not spawn enemies, effectively displaying game statically
        if self.is_display_genome_mode_on:
            return
        ...
        if self.spawnedEnemies != self.enemieslength:
            #shahar shortened time between enemy spawn:
            self.schedule_interval(self._add_enemy, self.spawningList[self.spawnedEnemies - 1][2]*resources.DURATION_MULTIPLIER)
        self._is_wave_finished()
    ...
    def on_key_press(self, k, m):
        if k == key.ESCAPE:
            ...
            return True
        if k == key.Q:
            director.replace(highscore.PyFenseLost(0,0,0,0))
        if k == key.B:
            import pdb
            pdb.set_trace()
            return True
