...
class PyFenseEnemy(sprite.Sprite, pyglet.event.EventDispatcher):
    ...
    def __init__(self, position, enemyname, lvl, wave, path,
        ...
        self.poisoned = 0
        #todo shahar shortened self._move delay:
        self.schedule_interval(self._move, 0.1* resources.DURATION_MULTIPLIER)
    ...
    def _move(self, dt):
        ...
        if self.distance != len(self.path[0]):
            ...
            # move the healthBar
            #healthBarAction = cocos.actions.MoveBy(self.path[1][self.distance], duration)
