...
class PyFenseLost(Scene):
    def __init__(self, current_wave, lives_left, earned, spent):
        super().__init__()
        self.wave = current_wave

        #shahar : added return value to director's interpreter locals and canceled the rest of the program
        director.interpreter_locals["current_wave"] = current_wave
        director.interpreter_locals["earned"] = earned
        director.interpreter_locals["lives_left"] = lives_left
        director.interpreter_locals["spent"] = spent
        director.pop()
