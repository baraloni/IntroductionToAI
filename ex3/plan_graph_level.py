...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        mtx = lambda x: previous_proposition_layer.is_mutex(x[0], x[1])
        for action in all_actions:
            if action in self.action_layer.get_actions():
                continue
            elif not previous_proposition_layer.all_preconds_in_layer(action):
                continue
            elif np.any(list(map(mtx, combinations(action.get_pre(), 2)))):
                continue
    ...
    def update_mutex_actions(self, previous_layer_mutex_proposition):
        ...
        actions_pairs = combinations(current_layer_actions, 2)
        for (a1, a2) in actions_pairs:
            if Pair(a1, a2) in self.action_layer.get_mutex_actions():
                continue
            elif mutex_actions(a1, a2, previous_layer_mutex_proposition):
    ...
    def update_proposition_layer(self):
        ...
        """
        # gathers this new layer's props and their producers:
        props_added = dict()
        current_layer_actions = self.action_layer.get_actions()
        for action in current_layer_actions:
            new_props = action.get_add()  # TODO: make sure that these are new propositions!
            for new_prop in new_props:
                if new_prop not in props_added:
                    props_added[new_prop] = []
                props_added[new_prop].append(action)

        # updates the layer's props and their producers:
        for prop in props_added:
            self.proposition_layer.add_proposition(prop)
            prop.set_producers(props_added[prop])
    ...
    def update_mutex_proposition(self):
        ...
        # checks for all 2 distinct props (without considering order) if they are mutexed
        # (len(current_layer_propositions) choose 2 options)
        proposition_pairs = combinations(current_layer_propositions, 2)
        for prop1, prop2 in proposition_pairs:
            if mutex_propositions(prop1, prop2, current_layer_mutex_actions):
                self.proposition_layer.add_mutex_prop(prop1, prop1)
    ...
    def expand(self, previous_layer):
        ...
        """
        # updates this level's action layer, and the mutex action:
        ...
        # update this level's props layer:
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    """
    for prop1 in a1.get_pre():
        for prop2 in a2.get_pre():
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    for a1 in producers1:
        for a2 in producers2:
            if (a1 != a2) and (not Pair(a1, a2) in mutex_actions_list):
                return False
    return False
